...
// ── Thai Corner Decoration (มุมซ้ายบน + ขวาล่าง) ──────────
function ThaiCorners({ size = 48 }: { size?: number }) {
  ...
  return (
    <>
      ...
      >
        {/* เส้นกรอบหลัก */}
        <path d="M2 24 L2 2 L24 2" fill="none" stroke="#ffd700" strokeWidth="2.5" strokeLinecap="round"/>
        {/* ลายกลีบบัว */}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* รูปใหญ่ + Thai corners */}
        <div className="relative">
          <div className="relative w-[520px] h-[390px] bg-surface-bg">
            <Image src={cert.imageUrl} alt={cert.name} fill className="object-contain p-4" />
          </div>
          <ThaiCorners size={64} />
        </div>
...
function CertCard({ cert, onClick }: { cert: Certificate; onClick: () => void }) {
  ...
  return (
    ...
    >
      {/* รูป + Thai corners แค่มุมซ้ายบน-ขวาล่าง */}
      <div className="relative w-full">
        <div className="relative w-full h-50 bg-surface-bg">
  ...
  return (
    <>
      ...
      <section id="certificates" className="section-padding">
        <h2 className="text-3xl font-bold text-slate-100 mb-2">Certificates</h2>
        <div className="w-12 h-1 mb-10" style={{
